			...
			if(args.length > 2) {
				for(OfflinePlayer p : Bukkit.getOfflinePlayers()) {
					if(p.getName().equalsIgnoreCase(args[0])) {
						...
						sender.sendMessage(COMAND_TAG+"§9"+p.getName()+" §7viens d'être mute pour : §9"+reason+" §8(durée : "+mute.getUntilTime()+"§7)");
						if(p.isOnline()) {
							p.getPlayer().sendMessage(COMAND_TAG+"§7Vous venez d'être mute pour : §9"+reason+" §8(durée : "+mute.getUntilTime()+"§7)");
						}
						return true;
					}
				}

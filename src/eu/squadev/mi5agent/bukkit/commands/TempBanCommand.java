...
public class TempBanCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lbl, String[] args) {
		if(sender.hasPermission("mi5agent.tempban")) {
			if(args.length > 1) {
				int timeCode[] = {0,0,0};
				int nbrArgs = 1;
				...
				if(timeCode[0] < 1 && timeCode[1] < 1 && timeCode[2] < 1) { 
					sender.sendMessage("§cErreur: /tempban <pseudo> <1d|1h|1m> [motif...]");
					return false;
				}else {
					...
					Mi5Agent.getManager().ban(args[0], reason, sender.getName(), cal);
					String sDate[] = ((BanTempory) Mi5Agent.getManager().getLastBan(args[0])).getUntilDateToString().split(" ");
					sDate = Mi5Agent.getManager().getFormatDate(sDate);
